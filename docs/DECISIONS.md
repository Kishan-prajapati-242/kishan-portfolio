...
- D-172: Education ships as a timeline section on the home page, after Teaching and before Papers. It sits on the home page rather than only on `/about` because for a co-op reader the degrees are a credential that supports everything above them, and Teaching and Education are both Khoury so they pair.
- D-173: The shape is borrowed from a reference portfolio, the implementation is not. That page uses a React timeline library, hotlinked university logos and cards arriving from alternating sides. Here the rail reuses the draw-on-scroll device already in `Divider.astro`, the cards are the same `Card` primitive and inherit the whole hover gesture, and the nodes are monograms.
- D-174: One rail on the left at every width, not alternating sides. Alternating works on the reference because that page is full-bleed. Here the content column is about 696px beside the 344px sidebar, so splitting it gives two 325px columns: measured, the cards became narrow towers and the rule stopped reading as a spine. Tried, screenshotted, rejected.
- D-175: No university logos. The reference hotlinks someone else's copies from a third-party GitHub Pages site. Logos are trademarks, and committing scraped images to a repository that gets sent to employers is a risk with no upside. Two-letter monograms in the site's own type carry the same identifying job. If Kishan wants real logos, he should supply files he has the right to use and they can drop into the node.
- D-176: Both GPAs are shown on the scale the awarding institution used. The undergraduate card leads with 8.32 out of 10 and gives 3.6 out of 4.0 as a stated equivalent, never the 3.6 alone. Presenting a converted figure as though a US institution issued it is exactly the kind of unlabelled number the rest of the site argues against, and a recruiter who checks the transcript finds the two agree.
- D-177: School-level education is deliberately excluded, unlike the reference which carries classes X and XII. It tells a co-op reader nothing and pushes the work further down the page.
- D-178: Content is drawn from CONTENT.md section 9 and nothing was invented, despite a request to pad it. The material is strong on its own: the ACL submission grew out of CS 6120, three of the four papers came out of the P P Savani lab, and the current GPA is carried while teaching three courses. Inventing detail on a site whose whole premise is measurement discipline is the failure mode CLAUDE.md names as the worst available, and it is also the first thing an interviewer probes.

## Analytics, and a divider that was never drawing

- D-179: Vercel Web Analytics added via `@vercel/analytics/astro` in `Base.astro`, so every route is covered by one component. It is a dependency beyond SPEC.md section 1, added on Kishan's explicit request. Page views and referrers only: cookieless, no cross-site identifier, nothing that needs a consent banner. Placed after the footer rather than in `<head>` so it stays out of the critical path, and outside `<main>` so it stays out of the element sweeps in the verify scripts.
- D-180: Cost is 1,302 bytes gzipped on every route, taking the worst route from 8,152 to 9,454 against the 16 KB ceiling, leaving 6,930. Off a Vercel deployment the script it fetches, `/_vercel/insights/script.js`, 404s, because that path is served by the platform edge rather than by the build; locally that is a failed request and nothing else.
- D-181: The `customElements.define` in the vendored component is called once per page load, and Astro's module caching means a client-side navigation does not re-run it. Verified across four consecutive client-side navigations: the element is still present, still defined, and no double-registration error is thrown. This was worth checking because a second `define` with the same name throws, and it would only have surfaced on navigation rather than on first load.
- D-182: `Divider.astro` was not drawing and had not been since D-094 reinstated it. The path was `M0.5 0.5 V8.5 M0.5 4.5 H100% M-0.5 0.5 V8.5`, and **percentages are not valid in SVG path data**: the parser rejects the `d` from `H100%` onward, so only the near cap ever rendered and every section divider on the site was a lone 8px tick rather than an error-bar bracket. Percentages *are* valid in the geometry attributes of `<line>` and friends, which is what makes the mistake easy to make.
  It is now three positioned boxes and a `scaleX`, which needs no path parser and reuses the same draw device as the scroll progress bar and the education rail rather than being a third technique. Verified: 696px wide, both 9px caps present, no console errors.
  Found only because the analytics work meant watching the console. Nothing in the existing gates catches it: the element is decorative and `aria-hidden`, so the visibility sweeps skip it, and it has no text for the contrast audit to measure. A console-error check across the routes would have caught it and is the obvious gap.

## Closing the gap D-182 named

- D-183: `scripts/verify-console.mjs` added. All 13 routes at 1440 and 390, each scrolled end to end so scroll-driven work and lazy content actually run, then four client-side navigations, with console, pageerror, requestfailed and HTTP status all attached. Errors and pageerrors fail the run; warnings print but do not, because a gate that fails on warnings gets muted and then the errors go unread with it.
- D-184: The Vercel analytics 404 is allowlisted by exact URL, `/_vercel/insights/script.js`, rather than by muting 404s as a class. Muting the class would hide a genuinely missing asset, which is the main thing this script is for.
- D-185: A console error for a failed resource carries only the text "Failed to load resource"; the URL is on `msg.location()`, not in the message. The first run reported the allowlisted analytics 404 thirty-six times, once per route and width, because the console handler could not see the URL to match it against the allowlist while the response handler could. The console handler now reads the location and appends it to the message, so every reported error names the resource it came from.
- D-186: The script includes a static scan for percentages inside SVG path data, which is exactly the D-182 bug and free to check: the path grammar takes numbers and nothing else, so a percent sign inside a literal `d` is always wrong and there is no false-positive case to weigh. `\bd\s*=` rather than `d=`, so `id=` and `data-d=` do not match. The limit, stated because it matters: it only sees literal attributes, so `d={icon.d}` and anything built from a template literal are invisible to it. It narrows the hole rather than closing it.
- D-187: The gate was validated against the known bug rather than assumed to work. Reintroducing the invalid path made it exit 1, name the file and line, and report the browser error at both widths; restoring the fix returned it to exit 0. A new gate that has only ever been run against a passing tree has not been shown to detect anything.
- D-188: Nothing else was found. Across 13 routes at two widths, with scrolling and client-side navigation, there are zero console errors and zero warnings. The divider was the only defect of that shape.
- D-189: `audit-motion.mjs` allowlists `marquee-left` and `marquee-right` by animation name in the time-clock section, with the reason inline. They had been reported on every run for many runs, and a permanent entry in a report is a report nobody reads. Allowlisted by name, not by ignoring infinite animations as a class, so a genuinely new never-ending animation still shows up on the first run after it is added.
